...
#  Related Topics Dynamic Programming 👍 1322 👎 77
...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        while query_row > 0:
            current_row = []
            end_val = max(float(0), (prev_row[0] - 1) / 2)
            current_row.append(end_val)
            for i in range(1, len(prev_row)):
                mid_val = max(float(0), (prev_row[i - 1] - 1) / 2) + max(float(0), (prev_row[i] - 1) / 2)
                current_row.append(mid_val)
            current_row.append(end_val)
            prev_row = current_row
            query_row -= 1
        return min(float(1), prev_row[query_glass])
