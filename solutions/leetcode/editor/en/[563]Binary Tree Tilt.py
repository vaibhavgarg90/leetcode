...
#  -1000 <= Node.val <= 1000 
#  
#  Related Topics Tree Depth-First Search Binary Tree 👍 1500 👎 1869
...
class Solution:
    def findTilt(self, root: Optional[TreeNode]) -> int:
        ...
        def rec(root):
            if not root:
                return 0, 0
            l_tilt, l_sum = rec(root.left)
            r_tilt, r_sum = rec(root.right)
            return l_tilt + r_tilt + abs(l_sum - r_sum), l_sum + r_sum + root.val
