...
#  -2³¹ <= xstart < xend <= 2³¹ - 1 
#  
#  Related Topics Array Greedy Sorting 👍 2621 👎 85
...
class Solution:
    def findMinArrowShots(self, points: List[List[int]]) -> int:
        ...
        common_interval = points[0]
        count = 1
        for i, point in enumerate(points):
            if point[0] <= common_interval[1]:
                start = max(common_interval[0], point[0])
                end = min(common_interval[1], point[1])
                common_interval = [start, end]
            else:
                common_interval = point
                count += 1
        return count
